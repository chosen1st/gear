...
int main(int argc, char const *argv[])
{
	...
	{
		while(count[i]!=0){
			cout<<"http://www.xuetangx.com/c4x/TsinghuaX/20740084X/asset/LinuxCpp__"<<setw(2)<<setfill('0')<<setiosflags(ios::right)<<i<<setw(2)<<setfill('0')<<setiosflags(ios::right)<<count[i]<<".pptx"<<endl;
			count[i]--;
		}
	}
	return 0;
